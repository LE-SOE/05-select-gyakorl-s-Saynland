...
function topPlayer() {
    ...
    let komolyak = document.getElementById("topJatekosok")
    /*for (let index = 0; index < 3; index++) {
        document.getElementById("topJatekosok").innerHTML += 
            `<li>${tmpGollovok[index].jatekos_neve} (${tmpGollovok[index].csapat_neve}) rúgott gólok: ${tmpGollovok[index].rugott_gol}</li>`
    }*/

   komolyak.innerHTML =''
    tmpGollovok.slice(0,3).forEach(x => {
        komolyak.innerHTML += `<li>${x.jatekos_neve} (${x.csapat_neve}) rúgott gólok: ${x.rugott_gol}</li>`
    })
}